...
function fromBase64(value: string): Uint8Array<ArrayBuffer> {
  return toCryptoBytes(Uint8Array.from(Buffer.from(value, 'base64')));
}
...
export async function encryptSecretKey(
  ...
  const cryptoApi = getCrypto();
  const salt = toCryptoBytes(cryptoApi.getRandomValues(new Uint8Array(SALT_LENGTH)));
  const iv = toCryptoBytes(cryptoApi.getRandomValues(new Uint8Array(IV_LENGTH)));
  ...
  const ciphertext = await cryptoApi.subtle.encrypt(
    ...
    encryptionKey,
    toCryptoBytes(new TextEncoder().encode(secretKey))
  );
