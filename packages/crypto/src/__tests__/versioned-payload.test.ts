...
describe('Versioned Payload Tests', () => {
  ...
  describe('Version Header Validation', () => {
    ...
    it('rejects unsupported version with UnsupportedVersionError', async () => {
      ...
      await expect(decryptSecretKey(futureVersionPayload, password))
        .rejects
    ...
    it('provides detailed error information for unsupported version', async () => {
      ...
      const error = await decryptSecretKey(futureVersionPayload, password)
        .catch(err => err) as UnsupportedVersionError;
    ...
    it('rejects negative version numbers', async () => {
      ...
      await expect(decryptSecretKey(negativeVersionPayload, password))
        .rejects
        .toThrow(UnsupportedVersionError);
    });
    ...
    it('rejects zero version', async () => {
      ...
      await expect(decryptSecretKey(zeroVersionPayload, password))
        .rejects
        .toThrow(UnsupportedVersionError);
    });
    ...
    it('rejects very large version numbers', async () => {
      ...
      await expect(decryptSecretKey(largeVersionPayload, password))
        .rejects
        .toThrow(UnsupportedVersionError);
    });
  });
  ...
  describe('Parser Validation', () => {
    it('rejects payload with missing version field', async () => {
      ...
      await expect(decryptSecretKey(payloadWithoutVersion as any, password))
        .rejects
    ...
    it('rejects payload with non-integer version', async () => {
      ...
      await expect(decryptSecretKey(floatVersionPayload, password))
        .rejects
        .toThrow(InvalidPayloadError);
    });
    ...
    it('rejects payload with string version', async () => {
      ...
      await expect(decryptSecretKey(stringVersionPayload, password))
        .rejects
        .toThrow(InvalidPayloadError);
    });

    it('rejects null payload', async () => {
      await expect(decryptSecretKey(null as any, password))
        .rejects
        .toThrow(InvalidPayloadError);
    });

    it('rejects undefined payload', async () => {
      await expect(decryptSecretKey(undefined as any, password))
        .rejects
        .toThrow(InvalidPayloadError);
    });

    it('rejects non-object payload', async () => {
      await expect(decryptSecretKey('not-an-object' as any, password))
        .rejects
        .toThrow(InvalidPayloadError);
    });
    ...
    it('provides specific error for missing salt', async () => {
      ...
      const error = await decryptSecretKey(payloadWithoutSalt as any, password)
        .catch(err => err) as InvalidPayloadError;
    ...
    it('provides specific error for empty salt', async () => {
      ...
      const error = await decryptSecretKey(emptySaltPayload, password)
        .catch(err => err) as InvalidPayloadError;
    ...
    it('provides specific error for missing iv', async () => {
      ...
      const error = await decryptSecretKey(payloadWithoutIv as any, password)
        .catch(err => err) as InvalidPayloadError;
    ...
    it('provides specific error for missing ciphertext', async () => {
      ...
      const error = await decryptSecretKey(payloadWithoutCiphertext as any, password)
        .catch(err => err) as InvalidPayloadError;
    ...
    it('provides specific error for invalid iterations range', async () => {
      ...
      const error = await decryptSecretKey(invalidIterationsPayload, password)
        .catch(err => err) as InvalidPayloadError;

      expect(error).toBeInstanceOf(InvalidPayloadError);
      expect(error.message).toContain('Payload iterations must be a safe integer between 100000 and 600000');
    });
    ...
    it('provides specific error for non-integer iterations', async () => {
      ...
      const error = await decryptSecretKey(floatIterationsPayload, password)
        .catch(err => err) as InvalidPayloadError;
  ...
  describe('Error Type Differentiation', () => {
    it('distinguishes between version errors and other payload errors', async () => {
      ...
      const payloadError = await decryptSecretKey(
        { ...encrypted, salt: '' }, 
        password
      ).catch(err => err);
    ...
    it('wraps crypto errors as InvalidPayloadError', async () => {
      ...
      const cryptoError = await decryptSecretKey(encrypted, 'wrong-password')
        .catch(err => err);
