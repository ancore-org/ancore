...
describe('protocol', () => {
  describe('isExternalRequest', () => {
    ...
    it('rejects malformed payloads', () => {
      // Missing type
      expect(
        isExternalRequest({ source: WALLET_API_SOURCE, requestId: 'abc', method: 'getAddress' })
      ).toBe(false);
      ...
      // Missing source
      expect(
        isExternalRequest({ type: ANCORE_WALLET_REQUEST, requestId: 'abc', method: 'getAddress' })
      ).toBe(false);
  ...
  describe('isExternalResponse', () => {
    ...
    it('rejects malformed response payloads', () => {
      // Missing type
      expect(
        isExternalResponse({ source: CONTENT_SCRIPT_SOURCE, requestId: 'abc', ok: true })
      ).toBe(false);
      ...
      // Missing source
      expect(isExternalResponse({ type: ANCORE_WALLET_RESPONSE, requestId: 'abc', ok: true })).toBe(
        false
      );
