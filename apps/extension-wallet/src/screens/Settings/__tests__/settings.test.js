...
describe('SettingsGroup', () => {
  it('renders title and children', () => {
    render(
      _jsx(SettingsGroup, {
        title: 'Network',
        children: _jsx(SettingItem, { label: 'Network', value: 'Testnet' }),
      })
    );
...
describe('SettingItem', () => {
  it('renders as button when onClick provided', () => {
    const onClick = vi.fn();
    render(_jsx(SettingItem, { label: 'Change Password', onClick: onClick }));
    const btn = screen.getByRole('button', { name: /change password/i });
    fireEvent.click(btn);
    expect(onClick).toHaveBeenCalledOnce();
  });
  it('applies danger styling', () => {
    render(_jsx(SettingItem, { label: 'Export Key', danger: true, onClick: vi.fn() }));
...
describe('NetworkSettings', () => {
  it('shows current network as active', () => {
    render(_jsx(NetworkSettings, { value: 'testnet', onChange: vi.fn(), onBack: vi.fn() }));
    expect(screen.getByText('Testnet')).toBeInTheDocument();
  ...
  it('switches to testnet without confirmation', async () => {
    ...
    const onBack = vi.fn();
    render(_jsx(NetworkSettings, { value: 'mainnet', onChange: onChange, onBack: onBack }));
  ...
  it('confirms mainnet switch', async () => {
    const onChange = vi.fn();
    const onBack = vi.fn();
    render(_jsx(NetworkSettings, { value: 'testnet', onChange: onChange, onBack: onBack }));
    await userEvent.click(screen.getByText('Mainnet'));
  ...
  it('cancels mainnet switch', async () => {
    const onChange = vi.fn();
    render(_jsx(NetworkSettings, { value: 'testnet', onChange: onChange, onBack: vi.fn() }));
    await userEvent.click(screen.getByText('Mainnet'));
  ...
  it('calls onBack when back button clicked', async () => {
    const onBack = vi.fn();
    render(_jsx(NetworkSettings, { value: 'testnet', onChange: vi.fn(), onBack: onBack }));
...
describe('SecuritySettings', () => {
  ...
  it('renders security menu items', () => {
    render(_jsx(SecuritySettings, { ...defaultProps }));
  ...
  it('navigates to auto-lock view and selects option', async () => {
    ...
    render(_jsx(SecuritySettings, { ...defaultProps, onAutoLockChange: onAutoLockChange }));
...
describe('AboutScreen', () => {
  it('renders version and links', () => {
    render(_jsx(AboutScreen, { onBack: vi.fn() }));
  ...
  it('calls onBack', async () => {
    const onBack = vi.fn();
    render(_jsx(AboutScreen, { onBack: onBack }));
    await userEvent.click(screen.getByRole('button', { name: /go back/i }));
    expect(onBack).toHaveBeenCalled();
  });
});
