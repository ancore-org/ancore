...
const PBXPROJ_PATH = path.join(
   MOBILE_APP_ROOT,
   "ios/AncoreWallet.xcodeproj/project.pbxproj"
);
...
function readMobileVersion() {
   const pkg = JSON.parse(
      readFileSync(path.join(MOBILE_APP_ROOT, "package.json"), "utf8")
   );
...
function syncIos(version) {
   ...
   const buildNumberMatches = [
      ...source.matchAll(/CURRENT_PROJECT_VERSION = (\d+);/g),
   ];
...
function main() {
   ...
   console.log(`Synced native projects to version ${version} (Android versionCode ${versionCodeFromSemver(version)}).`);
}
